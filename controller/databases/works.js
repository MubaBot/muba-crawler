...
exports.getAllWorks = () => {
  return Works.find({}).sort('updatedAt').then(works => works).catch(err => console.log(err));
}

exports.removeWork = async (engine, keyword, mode) => {
  return Works.findOneAndRemove({ searchEngine: engine, keyword: keyword }).then(result => result).catch(err => err);
}

exports.removeWorkById = async (id) => {
  return Works.findOneAndRemove({ _id: id }).then(result => result).catch(err => err);
}
